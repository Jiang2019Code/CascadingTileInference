...
class RandomRotation(nn.Module):
    ...
    def forward(
        ...
        if target["boxes"].size()[0] > 0:
            box = target["boxes"]
            box[:, 0] = box[:, 0] + (box[:, 2] - box[:, 0]) / 2
            box[:, 1] = box[:, 1] + (box[:, 3] - box[:, 1]) / 2
            box[:, 2] = (box[:, 2] - box[:, 0]) * 2
            box[:, 3] = (box[:, 3] - box[:, 1]) * 2

            img = image.numpy().transpose((1, 2, 0))
            instance = np.asarray(box)
            segments = np.zeros((0, 100, 2), dtype=np.float32)
            clsList = []
            for i in range(target["boxes"].size()[0]):
                clsList.append(1)
            labels = {"img": img, "cls": np.array(clsList),
                      "instances": Instances(bboxes=instance, normalized=False, segments=segments, bbox_format="xywh")}
            randomPerspective = augment.RandomPerspective(degrees=180, translate=0., scale=0., shear=0, perspective=0.0)
            result = randomPerspective(labels)
            img = result["img"]
            transformed_image = img.transpose((2, 0, 1))

            bboxes = torch.tensor(result["instances"].bboxes)
            if bboxes.size()[0] == 0:
                target["labels"] = torch.tensor([], dtype=torch.int64)
                target["masks"] = torch.tensor(np.zeros((0, 512, 512)), dtype=torch.uint8)  # 否则报输入输出不匹配的错误，在框架中进行mask?
                target["area"] = torch.tensor([])
                target["iscrowd"] = torch.tensor([])
            target["labels"] = torch.tensor(result["cls"], dtype=torch.int64)
            target[
                "boxes"] = bboxes  # torch.tensor(np.concatenate((bboxes[...,0:1], bboxes[...,3:], bboxes[...,2:3], bboxes[...,1:2]),axis=-1))
    ...
    def forward(
        ...
        while True:
            ...
            for _ in range(self.trials):
                ...
                ious = torchvision.ops.boxes.box_iou(boxes,
                                                     torch.tensor([[left, top, right, bottom]], dtype=boxes.dtype,
                                                                  device=boxes.device))
...
class RandomZoomOut(nn.Module):
    def __init__(self, fill: Optional[List[float]] = None, side_range: Tuple[float, float] = (1.0, 4.0),
                 p: float = 0.5):
        super().__init__()
        if fill is None:
            fill = [0.0, 0.0, 0.0]
...
class SimpleCopyPaste(torch.nn.Module):
    ...
    def __repr__(self) -> str:
        ...
        return s
# print( type(np.zeros((0, 100, 2), dtype=np.float32)))
